...
def run_model(
    ...
    # Y0: DBCR aplica sobre el Crédito LP (no sobre peaje) → usar valor base directamente
    ...
    # ── Métricas ────────────────────────────────────────────────
    # VAN/MIRR/VAFF calculados sobre exactamente 20 años (Y0..Y19)
    # replicando la fórmula Excel: NPV(rate, C45:V45) y MIRR(C45:V45,...)
    flujo_20     = flujo[:20]
...
with tab2:
    ...
    fig3.update_layout(**PL, height=400, legend=dict(bgcolor="rgba(0,0,0,0)"))
...
with tab3:
    ...
    st.info(
        ...
        f"- **Año 1 (2027):** Tránsito = {UTEQ_ARRANQUE:,.0f} UTEQs × tarifa $ {float(tarifa_input):,.0f} → ingreso = **$ {sc['peaje'][1]/1e9:.2f} MM**.  \n"
