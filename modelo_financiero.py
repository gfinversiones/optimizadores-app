# ============================================================
# MODELO FINANCIERO PPP / CONCESIONES
# ============================================================
...
st.title("📊 Modelo Financiero PPP / Concesiones")
...
st.sidebar.subheader("📉 Tasa Técnica")

tasa_tecnica = st.sidebar.slider(
    "Tasa Técnica %",
    0.0,
    30.0,
    10.0
...
transito_ajustado = transito_inicial * (
    1 + sens_transito / 100
)
...
capex_ajustado = capex_base * (
    1 + sens_capex / 100
)

opex_pct_ajustado = opex_pct * (
    1 + sens_opex / 100
)
...
for year in years:
    ...
    traf = transito_ajustado * (
        (1 + crecimiento_transito / 100) ** year
    )

    ingreso = traf * tarifa_ajustada

    costo_opex = ingreso * (
        opex_pct_ajustado / 100
    )
...
for i, val in enumerate(negative):
    ...
    pv_neg += val / (
        (1 + tasa_tecnica / 100) ** i
    )

for i, val in enumerate(positive):

    fv_pos += val * (
        (1 + tasa_tecnica / 100) ** (anios - i)
    )

mirr = (
    (fv_pos / abs(pv_neg)) ** (1 / anios)
) - 1
